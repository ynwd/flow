...
init:          ## Init core project from templates
	.copilot/skills/init-core-project/scripts/init-core.sh --force

# ─── Scaffold ────────────────────────────────────────────
scaffold:      ## Scaffold a new feature module: make scaffold name=blog [layers="h s r m p f"] [ssr=--ssr]
	@.copilot/skills/new-feature-module/scripts/scaffold.sh $(name) $(ssr) $(layers)
...
rebuild:       ## Copy app.css.tmpl → app.css, rebuild CSS, restart server
	@cp .copilot/skills/init-core-project/templates/core/static/css/app.css.tmpl core/static/css/app.css 2>/dev/null; npm run build:css 2>&1 | tail -1; fuser -k 3000/tcp 2>/dev/null || true; go run . &
